...
export async function fetchUpcomingEvents(): Promise<EventRow[]> {
  ...
  const { data, error } = await getSupabase()
    .from("events")
    .select(
      "id,source,external_id,title,description,starts_at,ends_at,url,image_url,venue_name,venue_address,gmaps_url,city,genres,price_early,price_normal,currency",
    )
    .gte("starts_at", since)
